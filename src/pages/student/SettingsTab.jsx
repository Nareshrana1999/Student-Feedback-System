...
const SettingsTab = () => {
  ...
  const handlePasswordChange = () => {
    ...
    // Update password in localStorage for student
    ...
    if (!loggedInUser || loggedInUser.role !== 'student') {
      toast({
        title: 'Error',
        description: 'No student user is currently logged in.',
        variant: 'destructive',
      });
      return;
    }
    const userIndex = users.findIndex(u => u.id === loggedInUser.id && u.role === 'student');
    if (userIndex === -1) {
      toast({
        title: 'Error',
